...
public abstract class PermissionUtil {
    ...
    public static boolean isGroupPermissionGranted(Activitymain activity, String[] permission) {
    ...
    public static boolean isGroupPermissionGrantedd(Activitymain activity, String[] permission) {
        if( Build.VERSION.SDK_INT >= Build.VERSION_CODES.M ) {
            if (permission.length == 0) return false;
            for (String s : permission) {
                if (ActivityCompat.checkSelfPermission(activity, s) != PackageManager.PERMISSION_GRANTED) {
